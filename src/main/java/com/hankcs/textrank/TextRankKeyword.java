...
//        System.out.println(entryList);
    ...
    static class StopWordDictionary{
        public final static String[] unconclude = new String[]{" ","\n","《","》","。","因为","但是"};
        public static boolean contains(String input){
            if (input.length()<=1) return true;
            for(int i=0;i<unconclude.length;i++){
                if (unconclude.equals(input)) return true;
            }
            return false;
        }
